...
@section('content')
    <x-breadcrumb :values="[__('Category')]">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($category->foto_url)
                                        <img src="{{ $category->foto_url }}" alt="Foto Category" style="max-width: 100px;">
                                    @else
                                ...
                                <td>
                                    @if ($category->foto_sampul)
                                        <img src="{{ asset($category->foto_sampul) }}" alt="Foto Sampul Category"
                                ...
                                <td>
                                    <button class="btn btn-info btn-sm btn-edit" data-id="{{ $category->id_category }}"
                                        data-nama="{{ $category->nama }}" data-judul="{{ $category->judul }}"
                                        data-foto="{{ $category->foto }}" data-foto_sampul="{{ $category->foto_sampul }}"
                                        data-bs-toggle="modal" data-bs-target="#editModal">
                                        {{ __('menu.general.edit') }}
                                    </button>
                                    <form action="{{ route('master.category.destroy', $category) }}" class="d-inline"
                                        method="post">
                                        @csrf
                                        @method('DELETE')
    ...
    <div class="modal fade" id="createModal" data-bs-backdrop="static" tabindex="-1">
        <div class="modal-dialog">
            <form class="modal-content" method="post" action="{{ route('master.category.store') }}"
                ...
                <div class="modal-body">
                    ...
                    <div class="col-sm-12 col-12 col-md-6 col-lg-4">
                        <div class="mb-3">
                            ...
                            <input type="file" class="form-control @error('foto') is-invalid @enderror" id="foto_sampul"
                                name="foto_sampul" />
